...
;; FSH grammar, ported from FSH.g4 (FHIR/sushi). The start rule is a single `entity`;
;; parse.rkt pre-splits a document into entities at top-level declaration keywords and
;; handles parameterized-RuleSet *definitions* separately (their body uses ANTLR token-set
;; negation `~(...)`, which brag can't express).
;;
;; Declaration keywords (Profile:, Parent:, …) and pure punctuation (*, =, :, ,, ->) are
;; hidden with `/` since the rule names carry their meaning; operator/flag/strength/boolean
;; keywords are KEPT because they distinguish rule variants.

entity:             alias | profile | extension | invariant | instance | valueSet
                  | codeSystem | ruleSet | mapping | logical | resource
...
sdRule:             cardRule | flagRule | valueSetRule | fixedValueRule | containsRule
                  | onlyRule | obeysRule | caretValueRule | insertRule | pathRule
...
ruleSetRule:        sdRule | addElementRule | addCRElementRule | concept
...
value:              STRING | MULTILINE_STRING | NUMBER | DATETIME | TIME | reference
                  | canonical | code | quantity | ratio | bool | name
...
mostAlphaKeywords:  KW_MS | KW_SU | KW_TU | KW_NORMATIVE | KW_DRAFT | KW_FROM | KW_CONTAINS
                  | KW_NAMED | KW_AND | KW_ONLY | KW_OR | KW_OBEYS | KW_TRUE | KW_FALSE
                  | KW_INCLUDE | KW_EXCLUDE | KW_CODES | KW_WHERE | KW_VSREFERENCE
                  | KW_SYSTEM | KW_CONTENTREFERENCE
